...
try
{
	...
	ResultSet rs= smt.executeQuery("Select Sum(total) From Cart where email='"+email+"' and status='bill'");
	while(rs.next()){
		total=rs.getInt(1);
	}
	
	ResultSet rs1=smt.executeQuery("Select * from Users Inner Join Cart where cart.email='"+email+"' and cart.status='bill'");
	while(rs1.next()){
%>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs2=smt.executeQuery("Select * From Cart Inner join Product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs2.next()){
	  sno+=1;
  %>
  <tr>
    <td><%= sno %></td>
    <td><img class="img1" src="images/<%=rs2.getString(21)%>"></td>
