...
try{
	...
	ResultSet rs = smt.executeQuery("Select Sum(total) From Cart Where email='"+email+"' and address is Null");
	while(rs.next()){
		total = rs.getInt(1);
	}
%>
        ...
        <tbody>
        <%
       ResultSet rs1 = smt.executeQuery(" Select * from product inner join cart on product.id = cart.product_id and cart.email='"+email+"' and cart.address is null ");
      	while(rs1.next())
      	{
      %>
          <tr>
          <% sno = sno+1; %>
          <td><%= sno %></td>
          <td><img class="img1" src="images/<%=rs1.getString(6)%>"></td>
...
<hr style="width: 100%">
<form action="addreesPaymentForOrderAction.jsp" method="post">
 <div class="left-div">
 <h3>Enter Address</h3>
...
<div class="right-div">
<h3>Pay online on this ecommerce@pay.com</h3>
...
<button class="button" type="submit">Proceed to Genertate Bill & Save <i class='far fa-arrow-alt-circle-right'></i></button>
